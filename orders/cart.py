...
class CartManager:
    ...
    def _mode_conflict_message(self, requested_mode):
        if requested_mode == CART_MODE_WHOLESALE:
            return (
                'В корзине уже есть розничный заказ.\n'
                'Чтобы перейти к оптовой покупке, сначала очистите текущую корзину.'
            )
        return (
            'В корзине уже есть оптовый заказ.\n'
            'Чтобы перейти к розничной покупке, сначала очистите текущую корзину.'
        )
