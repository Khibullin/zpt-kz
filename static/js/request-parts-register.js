...
el('sellerForm').addEventListener('submit',async e=>{e.preventDefault();let password=el('password').value;let password2=el('password2').value;if(password.length<8){setMessage('Пароль минимум 8 символов','error');return;}if(password!==password2){setMessage('Пароли не совпадают','error');return;}let payload={name:el('name').value.trim(),whatsapp:normalizePhone(el('whatsapp').value),password,password_confirm:password2,transport_type:el('transport_type').value,city:el('city').value,market_location:el('market_location').value,selected_category_ids:checkedIds('catcheck'),selected_country_ids:checkedIds('countrycheck'),selected_brand_ids:checkedIds('brandcheck'),selected_model_ids:checkedIds('modelcheck'),all_categories:el('all_categories').checked,all_countries:el('all_countries').checked,all_brands:el('all_brands').checked,all_models:el('all_models').checked,whatsapp_marketing_consent:el('whatsapp_marketing_consent').checked};let r=await fetch(API+'/create-seller/',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),credentials:'include'});let data=await r.json();if(data.error){setMessage(data.error,'error');return;}setMessage('Регистрация успешна. Теперь можно войти в кабинет продавца.','success');});
loadBase();
